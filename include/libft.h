...
# define FLAG_HASH			(1 << 0)  // 1
# define FLAG_ZERO			(1 << 1)  // 2
# define FLAG_MINUS			(1 << 2)  // 4
# define FLAG_PLUS			(1 << 3)  // 8
...
/* A struct for storing double-precision 2D coordinates. */
...
/* Format specifiers for ft_printf. */
...
/* Printf internal state tracking. */
typedef struct s_pf
{
	int		total;        /* Total characters written */
	int		write_error;  /* Flag set on write failure */
	int		malloc_error; /* Flag set on memory allocation failure */
	t_fmt	*fmt;         /* Current format spec */
}	t_pf;
...
/* ************************************************************************** */
/*
   Functions are grouped by topic or folder (array, list, matrix, check, mem,
   write, string/number conversions, string ops, get_next_line, utilities, math,
   vectors, ft_printf, etc.).
*/
