 ...
 */
static void insertion_sort(void* base, size_t start, size_t end, size_t size,
                           int (*cmp)(const void*, const void*))
{
	...
	while (i <= end)
	{
		...
		{
			ft_memcpy(get_elem(base, j + 1, size), get_elem(base, j, size),
			          size);
			j--;
		}
 ...
 */
static size_t median_of_three(void* base, size_t start, size_t end, size_t size,
                              int (*cmp)(const void*, const void*))
{
	size_t mid;
...
/**
 * @brief Recursive quicksort with insertion sort fallback and tail-call
 * optimization.
 *
 * @details
